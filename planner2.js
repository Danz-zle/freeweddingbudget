(() => {
  ...
  const render = () => {
    ...
    document.getElementById("p2BudgetSource").innerHTML = state.budget.importedAt
      ? `<strong>Migrated planner budget</strong><span>Your earlier calculator snapshot and subsequent Planner 2.0 changes are saved in this browser.</span>`
      : `<strong>Planner budget</strong><span>Your plan is saved privately in this browser.</span>`;
  ...
  document.getElementById("p2PaymentForm").addEventListener("submit", event => {
    ...
    state.payments.push({ id: id("payment"), name: data.name.trim(), vendor: linkedVendor ? linkedVendor.name : vendorName, vendorId: linkedVendor ? linkedVendor.id : "", amount: numberValue(data.amount), dueDate: data.dueDate, paid: false });
    event.currentTarget.reset(); save(); render();
  });
